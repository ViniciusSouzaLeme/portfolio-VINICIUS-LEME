...
def calcular_tempo_medio_login(arquivo):
...
def encontrar_data_extremos(arquivo):
    datas = []
    with open(arquivo, 'r') as file:
        for linha in file:
            dados = linha.strip().split(',')
...
def encontrar_usuarios_extremos(arquivo):
    usuarios = []
    with open(arquivo, 'r') as file:
        for linha in file:
            dados = linha.strip().split(',')
...
def calcular_tempo_medio_login_geral(arquivo):
...
def main():
    ...
    while True:
        ...
        if opcao == '1':
            numero_usuarios = contar_usuarios(nome_arquivo)
            print(f"Número de usuários: {numero_usuarios}\n")
        elif opcao == '2':
            letras, digitos, alfanumerico = contar_senhas(nome_arquivo)
            ...
            tempo_medio_login = calcular_tempo_medio_login(nome_arquivo)
            ...
            data_mais_logins, data_menos_logins = encontrar_data_extremos(nome_arquivo)
            ...
            usuarios_mais_logam, usuarios_menos_logam = encontrar_usuarios_extremos(nome_arquivo)
            ...
            tempo_medio_geral = calcular_tempo_medio_login_geral(nome_arquivo)
